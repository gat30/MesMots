...
public class PreferencesActivity extends AopdsActivity {
	...
	private void onPreferencesChanged() {
		...
		try {
			getPreferencesManager().setLanguage(
					((Language) languageSpinner.getSelectedItem())
							.getAbreviation());
	...
	private void initGui() {
		...
		languageAdapter = new LanguageSpinnerAdapter(getApplicationContext(),
				R.layout.language_spinner_item, allLanguages);

		languageAdapter
				.setDropDownViewResource(R.layout.language_drop_down_spinner_item);

		languageSpinner = (Spinner) findViewById(R.id.preferencesLanguageSpinner);

		languageSpinner.setAdapter(languageAdapter);

		languageSpinner.setSelection(languageAdapter
				.getPosition(languageAdapter
						.getLanguageByAbreviation(getPreferencesManager()
								.getLanguage())));
