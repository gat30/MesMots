...
public class AccountActivity extends AopdsActivity {
	...
	public void initGui() {
		...
		{
			AopdsLogger.info(getClass().getName(), "id : "+ getUserManager().getUser().getId());
			AopdsLogger.info(getClass().getName(), "nom : "+ getUserManager().getUser().getLastName());
			AopdsLogger.info(getClass().getName(), "prenom : "+ getUserManager().getUser().getFirstName());
			AopdsLogger.info(getClass().getName(), "mail: "+ getUserManager().getUser().getEmail());
			AopdsLogger.info(getClass().getName(), "mdp : "+ getUserManager().getUser().getPassword());
	...
	public void saveModifUser()
	{
		...
		if (getUserManager().isUserPersisted())
		{
			AopdsLogger.info(getClass().getName(), "L'utilisateur est enregistre");
			AopdsLogger.info(getClass().getName(), "id : "+ getUserManager().getUser().getId());
			AopdsLogger.info(getClass().getName(), "nom : "+ getUserManager().getUser().getLastName());
			AopdsLogger.info(getClass().getName(), "mdp : "+ getUserManager().getUser().getPassword());
			...
			{
				AopdsServiceAuthorizationToken token = new AopdsServiceAuthorizationToken(getUserManager().getUser().getEmail(),recordedPW);
				...
				if (newPW.equals(confNewPW))
				{
					try
					{
						User user = client.modifyUser((int) getUserManager().getUser().getId(), fn, ln, newPW, token);
						...
						//console display
						AopdsLogger.info(getClass().getName(), "Vérification du mot de passe effectuee");
						AopdsLogger.info(getClass().getName(), "psw : "+password.getText());
						AopdsLogger.info(getClass().getName(), "newpassword :"+newPassword.getText());
						AopdsLogger.info(getClass().getName(), "firstName : "+firstName.getText());
						AopdsLogger.info(getClass().getName(), "lastName : "+lastName.getText());
					    	
						finish();
						
						Intent intent = new Intent(getApplicationContext(), AopdsLauncher.class);
