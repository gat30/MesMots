...
public class AopdsServiceReturnHandler {
	...
	public AopdsServiceReturnHandler(String source) throws AopdsServiceException,AopdsServiceMalformedResponseException, AopdsServiceServerException {
		this.source = source;
	...
	private void parseSource() throws AopdsServiceException,AopdsServiceMalformedResponseException, IOException, AopdsServiceServerException {
	        ...
	        switch (eventType) {
		        ...
		        case XmlPullParser.START_TAG: // found a tag
		        	...
		        	if ( currentTag.equals( TAG_ENVELOPPE ) ) {
		        		enveloppeFound = true;
		        		AopdsLogger.info(LOG_TAG, "Found envoloppe, start parsing entities.");
		        	} else {
		        		if ( enveloppeFound ) {
		        			if ( currentTag.equals( TAG_ARRAY ) ) {
			        			...
			        			AopdsLogger.info(LOG_TAG, "Found unknown tag '" + currentTag + "', ingnoring it.");
			        		}
		        		} else {
		        			AopdsLogger.info(LOG_TAG, "Found tag '" + currentTag + "' before the enveloppe, ignoring it.");
		        		}
        ...
        if ( !enveloppeFound ) {
        	throw new AopdsServiceMalformedResponseException(
				"Malformed xml response. No Enveloppe found.",
				source,
				null
			);
        } else {
	...
	private AopdsServiceServerException handleError(XmlPullParser xpp, String tagName) throws AopdsServiceMalformedResponseException, IOException {
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing Error (tagName = " + tagNameToUse + ").");
        ...
        while ( (eventType != XmlPullParser.END_DOCUMENT) && !stop ) {
	        ...
	        switch (eventType) {
		        case XmlPullParser.START_TAG: // found a tag
		        	...
		        	if ( currentTag.equals( TAG_ERROR_CODE ) ) {
		        		...
		        		AopdsLogger.info(LOG_TAG, "Unknown tag " + currentTag + " in error, ignoring it.");
		        	}
	        ...
	        if ( !stop ) {
	        	try {
					...
					throw new AopdsServiceMalformedResponseException(
						"Malformed xml for Error definition, cannot map to Exception class." + 
						"Current tag parsed was : " + currentTag + ", current value for this tag was : " + currentValue,
						source,
						e
					);
	...
	private User handleUser(XmlPullParser xpp, String tagName) throws AopdsServiceMalformedResponseException, IOException {
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing User (tagName = " + tagNameToUse + ").");
	        ...
	        switch (eventType) {
		        case XmlPullParser.START_TAG: // found a tag
		        	...
		        	if ( currentTag.equals( TAG_USER_ID ) ) {
		        		...
		        		AopdsLogger.info(LOG_TAG, "Unknown tag " + currentTag + " in user, ignoring it.");
		        	}
	...
	private Suggestion handleSuggestion(XmlPullParser xpp, String tagName) throws AopdsServiceMalformedResponseException, IOException {
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing Suggestion (tagName = " + tagNameToUse + ").");
	        ...
	        switch (eventType) {
		        case XmlPullParser.START_TAG: // found a tag
		        	...
		        	if ( currentTag.equals( TAG_SUGGESTION_ID ) ) {
		        		...
		        		res.setActionType( handleString(xpp, TAG_SUGGESTION_ACTION_TYPE) );
		        		...
		        		res.setPronunciationRecorded( handleBoolean(xpp, TAG_SUGGESTION_PRONONCIATION_SENT) );
		        	} else if ( currentTag.equals( TAG_SUGGESTION_ADMIN_DECISION) ) {
		        		res.setSynchroStatus( handleString(xpp, TAG_SUGGESTION_ADMIN_DECISION) );
		        		...
		        		res.getAdmin().setId( handleInt(xpp, TAG_SUGGESTION_ID_ADMIN) );
		        		...
		        		res.getHeadword().setId( handleInt(xpp, TAG_SUGGESTION_ID_HEADWORD) );
		        	} else if ( currentTag.equals( TAG_SUGGESTION_ID_DICTIONARY) ) {
		        		res.getDictionary().setCode( handleInt(xpp, TAG_SUGGESTION_ID_DICTIONARY) );
		        	} else if ( currentTag.equals( TAG_SUGGESTION_DICTIONARY) ) {
		        		res.setDictionary( handleDictionary(xpp, TAG_SUGGESTION_DICTIONARY) );
		        	} else {
		        		AopdsLogger.info(LOG_TAG, "Unknown tag " + currentTag + " in suggestion, ignoring it.");
		        	}
	...
	private String handleString(XmlPullParser xpp, String tagName) throws AopdsServiceMalformedResponseException, IOException {
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing string (tagName = " + tagNameToUse + ").");
		...
		try {
			...
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for string definition.",
				source,
				e
			);
		}
	...
	throws AopdsServiceMalformedResponseException, IOException{
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing int (tagName = " + tagNameToUse + ").");
		...
		try {
			...
			} else throw new AopdsServiceMalformedResponseException(
			...
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for int definition.",
				source,
				e
			);
		} catch (NumberFormatException e) {
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for int definition. Impossible to parse int from '" + xpp.getText() + "'",
				source,
				e
			);
		}
	...
	throws AopdsServiceMalformedResponseException, IOException{
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing boolean (tagName = " + tagNameToUse + ").");
		...
		try {
			...
			if ( eventType == XmlPullParser.TEXT ) {
				...
				if ( !(result == 1) && !(result == 0) ) {
					throw new AopdsServiceMalformedResponseException(
						"Malformed xml for int definition. Impossible to parse boolean from '" + xpp.getText() + "'",
						source,
						null
					);
				} 
	...
	throws AopdsServiceMalformedResponseException, IOException{
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing long (tagName = " + tagNameToUse + ").");
		
		try {
			eventType = xpp.next();
		} catch (XmlPullParserException e) {
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for long definition.",
				source,
				e
			);
		}
		...
		if ( eventType == XmlPullParser.TEXT ) {
			try {
				...
				throw new AopdsServiceMalformedResponseException(
					"Malformed xml for long definition. Impossible to parse long from '" + xpp.getText() + "'",
					source,
					e
				);
			}
	...
	private Dictionary handleDictionary(XmlPullParser xpp, String tagName) throws AopdsServiceMalformedResponseException, IOException {
		...
		try {
			...
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for dictionary definition.",
				source,
				e
			);
		}
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing Dictionary (tagName=" + tagNameToUse + ").");
	        ...
	        switch (eventType) {
		        case XmlPullParser.START_TAG: // found a tag
		        	...
		        	if ( currentTag.equals( TAG_DICTIONARY_LANGUAGE_FROM ) ) {
		        		res.setLanguageFrom( handleLanguage(xpp, TAG_DICTIONARY_LANGUAGE_FROM ) );
		        	} else if ( currentTag.equals( TAG_DICTIONARY_LANGUAGE_TO ) ) {
		        		res.setLanguageTo( handleLanguage(xpp, TAG_DICTIONARY_LANGUAGE_TO ) );
		        		...
		        		AopdsLogger.info(LOG_TAG, "Unknown tag " + currentTag + " found in dictionary, ignoring it.");
		        	}
	...
	private Language handleLanguage( XmlPullParser xpp, String tagName ) throws AopdsServiceMalformedResponseException, IOException {
		...
		try {
			...
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for Language definition.",
				source,
				e
			);
		}
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing Language (tagName = " + tagNameToUse + ").");
	        ...
	        switch (eventType) {
		        case XmlPullParser.START_TAG: // found a tag
		        	...
		        	if ( currentTag.equals( TAG_LANGUAGE_CODE ) ) {
		        		...
		        		res.setAbreviation( handleString(xpp, TAG_LANGUAGE_ABBREVIATION) );
		        	} else {
		        		AopdsLogger.info(LOG_TAG, "Unknown tag " + currentTag + " in language, ignoring it.");
		        	}
	...
	private String getXmlAttribute(XmlPullParser xpp, String attributeName) {
		...
		if ( attributeCount <= 0 ) return null;
			...
			if ( found ) return xpp.getAttributeValue(index);
			else return null;
		}
		
	}
	
	private Object handleArray(XmlPullParser xpp, String tagName) throws AopdsServiceMalformedResponseException, IOException{
		...
		try {
			...
			throw new AopdsServiceMalformedResponseException(
				"Malformed xml for array definition.",
				source,
				e
			);
		}
		...
		AopdsLogger.info(LOG_TAG, "Starting parsing Array(innerType=" + innerType + ") (tagName=" + tagNameToUse + ").");
	        ...
	        if ( !stop ) {
	        	try {
	    			...
	    			throw new AopdsServiceMalformedResponseException(
						...
						"Current tag parsed was : " + xpp.getName() + 
						", current value for this tag was : " + xpp.getText() + 
						", innerType for this array was : " + innerType,
	    				source,
	    				e
	    			);
	    		}
