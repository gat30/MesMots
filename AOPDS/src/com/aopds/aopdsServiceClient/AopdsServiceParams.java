...
public class AopdsServiceParams {
	...
	public void addAuthorizationToken(AopdsServiceAuthorizationToken token) {
		...
		BasicNameValuePair userId = new BasicNameValuePair(
				"aopdsAuthUserEmail", token.getUserId());

		BasicNameValuePair userPsw = new BasicNameValuePair(
				"aopdsAuthUserPassword", token.getUserPassword());
	...
	public void addString(String paramName, String paramValue) {
		BasicNameValuePair stringPair = new BasicNameValuePair(paramName,
				paramValue);
	...
	public void addUser(String paramName, User u) {
		...
		BasicNameValuePair userId = new BasicNameValuePair(paramName + "[id]",
				Long.toString(u.getId()));

		BasicNameValuePair userEmail = new BasicNameValuePair(paramName
				+ "[email]", u.getEmail());

		BasicNameValuePair userPassword = new BasicNameValuePair(paramName
				+ "[password]", u.getPassword());

		BasicNameValuePair userFirstName = new BasicNameValuePair(paramName
				+ "[firstName]", u.getFirstName());

		BasicNameValuePair userLastName = new BasicNameValuePair(paramName
	...
	public void addLongList(String paramName, List<Long> li) {
		...
		while (it.hasNext()) {
			...
			BasicNameValuePair value = new BasicNameValuePair(paramName + "["
					+ i + "]", Long.toString(current));
	...
	public void addSuggestionList(String paramName, List<Suggestion> ls) {
		...
		while (it.hasNext()) {
			...
			BasicNameValuePair idLocalSuggestion = new BasicNameValuePair(
					paramName + "[" + i + "][idLocalSuggestion]",
					Long.toString(current.getId()));

			BasicNameValuePair actionType = new BasicNameValuePair(paramName
					+ "[" + i + "][actionType]", current.getActionType());

			BasicNameValuePair word = new BasicNameValuePair(paramName + "["
					+ i + "][word]", current.getWord());

			BasicNameValuePair entry = new BasicNameValuePair(paramName + "["
					+ i + "][entry]", current.getEntry());

			BasicNameValuePair phonetic = new BasicNameValuePair(paramName
					+ "[" + i + "][phonetic]", current.getPhonetic());

			BasicNameValuePair prononciationSent = new BasicNameValuePair(
					paramName + "[" + i + "][prononciationSent]",
					Boolean.toString(current.getPronunciationRecorded()));

			BasicNameValuePair idDictionary = new BasicNameValuePair(paramName
					+ "[" + i + "][idDictionary]", Integer.toString(current
					.getDictionary().getCode()));

			BasicNameValuePair idHeadword;
			if (current.isAddActionType()) {
				idHeadword = new BasicNameValuePair(paramName + "[" + i
						+ "][idHeadword]", "null");
			} else {
				idHeadword = new BasicNameValuePair(paramName + "[" + i
						+ "][idHeadword]", Long.toString(current.getHeadword()
						.getId()));
			}
