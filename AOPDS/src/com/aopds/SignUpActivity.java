...
public class SignUpActivity extends AopdsActivity {
	...
	private void initGUI() {
		...
		signUp.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				 ...
				 * Returns an object indicating whether there has been a user
				 * error or not and what kind of error it is.
				 */
				AopdsFormError errorinform = checkForm();

				AopdsLogger.info(getClass().getSimpleName(), "error in form? "
						+ Boolean.toString(errorinform.getErrorcode()));
	...
	private boolean isEmail() {
		...
		boolean condition1 = email.getText().toString().indexOf("@") > 0; // true
																			// if
																			// the
																			// address
																			// contains
																			// a
																			// @
		boolean condition2 = email.getText().toString().indexOf(".") > 0; // true
																			// if
																			// the
																			// address
																			// contains
	...
	/*
	 * checks the signup form and returns an object that contains - a boolean
	 * saying if the form is well filled - a string saying what is wrong if
	 * something is wrong
	 */
	private AopdsFormError checkForm() {
		...
		if (firstName.getText().toString().compareTo("") == 0) {
			...
			error.addToErrordisplay(getString(R.string.LABEL_EMPTY_BLANK) + " "
					+ getString(R.string.LABEL_FIRST_NAME) + ".\n");
		...
		if (lastName.getText().toString().compareTo("") == 0) {
			...
			error.addToErrordisplay(getString(R.string.LABEL_EMPTY_BLANK) + " "
					+ getString(R.string.LABEL_LAST_NAME) + ".\n");
			error.setErrorcode(true);
		}
		...
		if (!isEmail()) {
			...
			error.addToErrordisplay(getString(R.string.LABEL_UNCORRECT_ENTRY)
					+ " " + getString(R.string.LABEL_YOUR_EMAIL_ADDRESS)
					+ ".\n");
			error.setErrorcode(true);
		}

		if (email.getText().toString()
				.compareTo(emailConfirm.getText().toString()) != 0) {
	...
	private void saveUser() {
		AopdsUserManager userManager = AopdsUserManager
		...
		try {
			...
			User user = client.signUpUser(email.getText().toString(), psw
			...
			AopdsLogger
					.info(getClass().getName(), "email : " + email.getText());
	...
	/*
	 * private void fillSpinner( Spinner s) { AopdsDatabase db=
	 * AopdsDatabase.getInstance( getApplicationContext() );
	 * ArrayAdapter<String> adapterForSpinner;
	 * 
	 * ArrayList<Language> allLanguage; try {
	 * 
	 * allLanguage = db.getAllLanguages(); Iterator<Language> il =
	 * allLanguage.iterator();
	 * 
	 * 
	 * adapterForSpinner = new
	 ...
	 * 
	 * 
	 * while (il.hasNext()) { adapterForSpinner.add(il.next().getName()); }
	 * 
	 * } catch (AopdsDatabaseException e) { e.printStackTrace(); } }
	 */
}