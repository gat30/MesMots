...
public class SynchronizeActivity extends AopdsActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// adapter = new SuggestionListAdapter(getApplicationContext(),
		// R.layout.suggestion_list_item, sl);
		adapter = new WordListAdapter(getApplicationContext(),
				R.layout.search_word_list_item, sl, null);
		initGui();
	}
	...
	public void initGui() {
		...
		try {
			...
			ArrayList<Suggestion> modifyDeleteSuggestions = data
					.getAllModifyDeleteSuggestions();
			...
			if (u != null && u.getEmail() != null && u.getEmail() != null) {
				...
				if (allSugg != null && allSugg.size() >= nbSuggServer) {
					...
					while (it.hasNext()) {
						...
						if (synchroStatut
								.equals(Suggestion.SYNCHRO_STATUS_UNSYNCHRONIZED)) {
							current.setSynchroStatus(Suggestion.SYNCHRO_STATUS_WAITING);
							unSynchronized.add(current);
						} else {
							if (synchroStatut
									.equals(Suggestion.SYNCHRO_STATUS_WAITING)) {
								pending.add(current.getId());
							}

						}
					}
					// AopdsLogger.info("SynchroActuvity",
					// unSynchronized.toString());

					// Server Synchronisation
					List<Suggestion> returnedList = client
							.synchronizeSuggestions(unSynchronized, pending,
									token, u.getId());

					// Updating the synchro_status in the database from
					// unsynchronized (u) to pending (p)
					unSynchronizedListIterator = unSynchronized.iterator();
					while (unSynchronizedListIterator.hasNext()) {
						Suggestion currentUnSynchronized = unSynchronizedListIterator
								.next();
						...
						while (ite.hasNext()) {
							...
							if (current.getId() == -1) {
								...
								if (current.isAddActionType()) {
									current = data.getAddSuggestion(current
											.getId());
								} else {
									current = data
											.getModifyDeleteSuggestion(current
													.getId());
								}
					...
					allMySuggFromServer = (ArrayList<Suggestion>) client
							.getAllSuggestion((int) u.getId(), token);
					// AopdsLogger.info("LA LISTE",
					// allMySuggFromServer.toString());
